...
namespace PR_3
{
    ...
    {
        private string watermarkText = "от 0 до 5";
        public Form1()
        {
            InitializeComponent();
            ...
            // Инициализация
            textBoxColumns.Text = watermarkText;
            textBoxColumns.ForeColor = Color.Gray;

            textBoxLines.Text = watermarkText;
            textBoxLines.ForeColor = Color.Gray;

            // Обработчик события GotFocus для удаления подсказки
            textBoxColumns.GotFocus += (sender, e) =>
            {
                if (textBoxColumns.Text == watermarkText)
                {
                    textBoxColumns.Text = "";
                    textBoxColumns.ForeColor = Color.Black; // Верните цвет по умолчанию
                }
            };
            textBoxLines.GotFocus += (sender, e) =>
            {
                if (textBoxLines.Text == watermarkText)
                {
                    textBoxLines.Text = "";
                    textBoxLines.ForeColor = Color.Black; // Верните цвет по умолчанию
                }
            };

            // Обработчик события LostFocus для восстановления подсказки
            textBoxColumns.LostFocus += (sender, e) =>
            {
            ...
            textBoxLines.LostFocus += (sender, e) =>
            {
        ...
        private void textBoxColumns_TextChanged(object sender, EventArgs e)
        {
            ...
            if (textBoxColumns.Text == "от 0 до 5")
            {
                errorProvider.Clear();
                return; // Прерываем выполнение, если всё норм
            }
        ...
        private void textBoxLines_TextChanged(object sender, EventArgs e)
        {
            ...
            if (textBoxLines.Text == "от 0 до 5")
            {
                errorProvider.Clear();
                return; // Прерываем выполнение, если всё норм
            }
        ...
        private void textBoxColumns_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrEmpty(textBoxColumns.Text))
            {
                // Вывод серого текста, который нельзя скопировать
                textBoxColumns.ForeColor = Color.Gray;
                textBoxColumns.Text = "от 0 до 5";
                textBoxColumns.ReadOnly = true;
            }
        }

        private void textBoxColumns_Validated(object sender, EventArgs e)
        {
            if (textBoxColumns.Text == "от 0 до 5")
            {
                // Очистка поля, если текст был "от 0 до 5"
                textBoxColumns.Text = "";
                textBoxColumns.ReadOnly = false;
                textBoxColumns.ForeColor = Color.Black;
            }
        }

        private void textBoxLines_Validating(object sender, CancelEventArgs e)
        {
            if (string.IsNullOrEmpty(textBoxLines.Text))
            {
                // Вывод серого текста, который нельзя скопировать
                textBoxLines.ForeColor = Color.Gray;
                textBoxLines.Text = "от 0 до 5";
                textBoxLines.ReadOnly = true;
            }
        }

        private void textBoxLines_Validated(object sender, EventArgs e)
        {
            if (textBoxLines.Text == "от 0 до 5")
            {
                // Очистка поля, если текст был "от 0 до 5"
                textBoxLines.Text = "";
                textBoxLines.ReadOnly = false;
                textBoxLines.ForeColor = Color.Black;
            }
        }
    }
}